# ran statistics on the main code that gave me 87% r2 score
...
# Read the data
data = pd.read_csv("NUMERICAL_DATA_copy_1.csv")  # Data for first model (ARR_DELAY prediction)
data1 = pd.read_csv("NUMERICAL_DATA_copy_2.csv")  # Data for second model (DEP_DELAY prediction)

# First model: Predicting ARR_DELAY using 'data'
...
# Predict ARR_DELAY for the entire 'data' dataset and add it to 'data1'
data1['Predicted_ARR_DELAY'] = cross_val_predict(pipeline_1, data[featuresX_1], y_1, cv=5)

# Second model: Predicting DEP_DELAY using 'data1' and predicted ARR_DELAY
