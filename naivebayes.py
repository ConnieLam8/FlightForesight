...
X_classification = df_sampled[features_delayed_status]
y_delayed_status = df_sampled["Delayed_Status"]
...
model_delayed_status = GaussianNB()
model_delayed_status.fit(X_train_class, y_train_class)
...
df_sampled['Predicted_Delayed_Status'] = model_delayed_status.predict(
    scaler_class.transform(df_sampled[features_delayed_status])
)

# Step 2: Train and evaluate the regression model for DEP_DELAY
X_regression = df_sampled[features_dep_delay]
y_dep_delay = df_sampled["DEP_DELAY"]
...
grouped_data = df_sampled.groupby(['Origin_DestPair', 'Delayed_Status'])['route_delay_rank'].median().reset_index()
...
example_input['average_airline_delay'] =example_input['DOT_CODE'].map(
    df_sampled.groupby('DOT_CODE')['average_airline_delay'].mean()
)
...
hourly_grouped = df_sampled.groupby(['Origin_DestPair', 'crs_dep_military_hour', 'Delayed_Status'])['hourly_origin_dest_average'].median().reset_index()
