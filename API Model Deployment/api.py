...
def result():
    if request.method == 'POST':
        ...
        # Give a key value to the dictionary for the corresponding column name
        print("Form Data (Key-Value Pairs):", to_predict_list)
        ...
        dot_code_value = int(to_predict_list.get('DOT_CODE'))
        
        # Extract origin and destination airport from the form change to int format for comparison
        origin_value = int(to_predict_list.get('ORIGIN'))
        dest_value = int(to_predict_list.get('DEST'))
        ...
        '''
        dep_value = to_predict_list.get('crs_dep_military_time')
        X_class_input['crs_dep_military_time'] = int(dep_value.replace(':', ''))

        arr_value = to_predict_list.get('crs_arr_military_time')
